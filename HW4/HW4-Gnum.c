...
//AVL Tree functions
struct address_t * rebalance(int balanceFactor, struct address_t * unbalancedNode);              
int findBalance(struct address_t * node);                           //works
struct address_t * rotateLeft(struct address_t * x);
struct address_t * rotateRight(struct address_t * x);
...
struct address_t * rebalance(int balanceFactor, struct address_t * unbalancedNode) {
    if (balanceFactor > 1) {
      // left-left imbalance
      if(findBalance(unbalancedNode->leftChild) >= 0) {
        unbalancedNode = rotateRight(unbalancedNode);
        return unbalancedNode;
      }

      // left-right imbalance
      else if(findBalance(unbalancedNode->leftChild) == -1) {
        unbalancedNode->leftChild = rotateLeft(unbalancedNode->leftChild);
        unbalancedNode = rotateRight(unbalancedNode);
        return unbalancedNode;
      }
    }
    ...
    if (balanceFactor < -1) {
      ...
      if(findBalance(unbalancedNode->rightChild) <= -0) {
        unbalancedNode = rotateLeft(unbalancedNode);
        return unbalancedNode;
      }

      // right-left imbalance 
      else if(findBalance(unbalancedNode->rightChild) == 1) {
...
struct address_t * rotateLeftThenRight(struct address_t * node) {
    if (findBalance(node->leftChild) == -1) {
...
struct address_t * rotateRightThenLeft(struct address_t * node) {
    if(findBalance(node->rightChild) == 1) {
...
struct address_t * rotateLeft(struct address_t * x) {
    struct address_t * y = x->rightChild;
    struct address_t * temp = y->leftChild;
...
struct address_t * rotateRight(struct address_t * x) {
    struct address_t * y = x->leftChild;
    struct address_t * temp = y->rightChild;
...
int findBalance(struct address_t * node) {
    if (node == NULL)
        return -1;
...
void displayList(struct address_t *root) {
    ...
    if (root != NULL) {
        ...
        if ( (root->depth == -1) || (root->height == -1) || (root->balance == -111) ) {
        ...
        if ( root->parent == NULL ) {
            // root->depth = findDepth(head, root->alias);
            printf("%d.%d.%d.%d %s height:%d depth:%d balance:%d parent:NONE\n", root->octet[0], root->octet[1], root->octet[2], root->octet[3], root->alias, root->height, root->depth, root->balance);
        } else {
            // root->depth = findDepth(head, root->alias);
            printf("%d.%d.%d.%d %s height:%d depth:%d balance:%d parent:%s\n", root->octet[0], root->octet[1], root->octet[2], root->octet[3], root->alias, root->height, root->depth, root->balance, root->parent->alias);
        }
        // if (root->parent == NULL)
        //     printf("Parent: NULL\n");
        // else
